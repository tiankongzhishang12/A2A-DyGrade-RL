...
克隆后继续复用 `autodl-a2a` Host别名，避免修改Codex Desktop Connection注册。新实例SSH密钥认证、远程路径、Git、模型hash和控制面已通过独立迁移run；克隆后第一次Desktop只读重连也已确认新hostname、项目路径、分支、HEAD和clean tree，连接身份门已关闭。
