...
**创建日期**：2026-07-04
**最后修订**：2026-07-29
**状态**：V1.4 职责与内部拆分正式规格稿（等待用户审阅后实施）
**输入**：用户确认 V1.3 质量协议继续有效，并进一步要求：`train_fit` 只训练参数，`train_calibration` 只为每个冻结 checkpoint 校准 STOP 安全边界并组装 Package，Dev 才选择最终 Router；内部数据不得直接拆现有 train Paper，必须先按题目组拆分原 train 主路由 Item，再分别重建 `train_fit/train_calibration` strict Paper。
...
> **先通过相对质量参考的零容忍配对统计非劣效门，再在质量可行的评分路径中寻找资源消耗更低的路径。**
...
`Quality Metric Protocol` 是在真实 Pilot 与 Router 结果前冻结的机器可读协议，至少包含：Gate Error 与未完成处理、Severe/Extreme 阈值、Unsafe Stop 分母和零 STOP 处理、Macro-NMAE、固定11档 QWK、QWK readiness、Bootstrap 参数及 Dev 词典序。

### 2.7 配对统计质量门

`Paired Bootstrap Gate` 在相同 split、Paper、Agent cache 和预算档位上，对候选与冻结参考策略使用相同 Paper 重采样索引，执行5000次单侧95%配对 Cluster Bootstrap。零非劣效界下，任一主指标未定义、置信区间跨0或未达到规定边界，均令 `Quality Feasible = No`。
...
研究者可以基于 `train_fit` 重建 Paper 和逐步暴露的 Agent cache 构建离线轨迹，训练带 Stop-Risk Head 的质量约束 Router；再在独立 `train_calibration` Paper 上为每个冻结 checkpoint 校准 STOP 安全边界，最后由 Dev 从完整 Package 中选择唯一方案，并与分类器、自动阈值、Bandit、greedy、knapsack 和固定多 Agent 工作流公平比较。

**优先级原因**：该故事提供论文核心证据，同时保证“参数学习、边界校准、最终 Router 选择”三件事使用不同数据职责，避免同一数据既调边界又宣称方案胜出。

**独立测试**：完成 `internal item split -> separate paper rebuild -> train_fit -> train_calibration package assembly -> dev paired quality gate/resource-first auto-select -> freeze -> test-like smoke`，验证 calibration 不训练参数、不排名 checkpoint，Dev 不移动边界。
...
7. **给定** 边界已冻结的多个预算条件 Policy Package，**当** 在 Dev 上选择，**则**程序先要求候选在 Tight/Medium/Loose 每个预算档位均通过质量门，再按跨预算等权资源顺序自动输出唯一 Package/checkpoint，不重新拟合或移动边界。
...
- **FR-017**：Dev 只能比较边界已经冻结的完整 Policy Package：先在所有预算档位执行 FR-047 质量门，再按 FR-048 的跨预算资源词典序自动选择唯一 Package/checkpoint；不得重新拟合、移动或试探边界。
...
- **FR-028**：系统必须保存所有候选 checkpoint、校准边界、质量协议 hash、readiness、Bootstrap 置信区间、Dev 资源排序、淘汰原因和失败状态。
- **FR-029**：没有候选通过质量门时，系统必须返回失败状态而不是放宽阈值、改指标或更换参考。
...
- **FR-035**：Baseline 至少包含固定 Agent、自动校准阈值、静态分类器、固定级联、完整多 Agent、Contextual Bandit、myopic/greedy 和 Top-k/knapsack。
- **FR-036**：消融至少覆盖无 A2A、无预算状态、无能力画像、无 Stop-Risk Head、无 CAG 图编码和无自动风险校准。
- **FR-037**：系统必须报告 Quality Feasible、Gate Error 处理、Severe/Extreme Error、Unsafe Stop、Stop Coverage、Deferral、dataset QWK、Macro-QWK、Macro/Micro-NMAE、QWK readiness、Bootstrap 边界、资源、通信、预算耗尽和失败状态。
...
- **FR-046**：质量门必须使用 Paper 为 cluster、候选/参考配对、5000次、单侧95%、零非劣效界、固定种子 `20260729` 的 Bootstrap；同一重采样中候选与参考必须共享 Paper 索引。
...
- **FR-048**：同一个预算条件 Policy Package 必须在所有预注册预算档位分别通过 FR-047 才具备 Package 级质量可行性。Dev 只能在此类候选中按三个预算档位等权平均的 `Cost/Paper -> Elapsed Time/Paper -> Agent Calls/Paper -> A2A Exchanges/Paper` 排序，再以跨预算最坏 Severe、跨预算最坏 Unsafe Stop、跨预算平均 Macro-NMAE、跨预算平均 Macro-QWK 和 Policy Package ID 打破并列；最终只冻结一个 Package/checkpoint，且不得结果后改变聚合或顺序。
...
- **Paired Bootstrap Gate Result**：候选—参考指标差值、Paper 重采样配置、单侧置信边界、四项通过状态和失败原因。
...
- **SC-005**：质量指标协议、质量参考、STOP 安全概率边界、预算档位和策略包选择均由预注册程序产生或冻结；calibration 与 Dev 各自生成职责独立的可审计 manifest。
- **SC-006**：Dev 不含人工操作；相同输入、种子和候选集合的两次运行，对每个预算档位输出相同质量门状态，并输出相同的跨预算聚合排序和唯一预算条件 Policy Package/checkpoint。
...
- **SC-009**：在同一 split、Paper、Agent cache 和预算档位上，候选仅在 `UCB95(max_dataset_delta_severe) <= 0`、`UCB95(max_dataset_delta_unsafe_stop) <= 0`、`UCB95(delta_macro_nmae) <= 0` 和 `LCB95(delta_macro_qwk) >= 0` 四项同时成立时标记为 `Quality Feasible = Yes`。
- **SC-010**：若不存在质量可行策略，系统输出明确失败状态并保留全部候选、readiness、置信区间和排序记录，不修改质量门。
- **SC-011**：主报告完整包含固定 Agent、自动阈值、分类器、Bandit、greedy/knapsack、完整多 Agent 和质量约束 CAG-CQL。
...
- **SC-019**：候选必须在 Tight/Medium/Loose 全部通过质量门；随后严格按跨预算等权平均的 `Cost/Paper -> Elapsed Time/Paper -> Agent Calls/Paper -> A2A Exchanges/Paper` 及固定质量并列规则自动排序，最终只冻结一个预算条件 Policy Package/checkpoint，资源节省声明只出现在全部预算档位质量可行的结果上。
...
- **SC-023**：Dev 输入的所有 Package 边界在进入 Dev 前已冻结；Dev 边界修改次数为0，并且相同候选、种子和 manifests 两次运行输出相同的唯一最终 Package/checkpoint。
...
- 每个预注册预算档位独立执行参考选择与配对质量门；Dev 在全部档位均质量可行的候选中按跨预算等权资源聚合自动选择并冻结唯一预算条件 Policy Package/checkpoint。
