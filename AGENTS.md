...
### III. 评分质量优先，资源不可补偿质量失败

评分质量和严重错分风险是本项目的第一评价层；成本、累计延迟、Agent 调用次数和 A2A Exchanges 是第二评价层。正式候选必须先通过相对于固定参考策略的零容忍配对统计准入门，再通过以 `Quality Champion` 为基准的质量保护门，之后才允许比较资源消耗。

任何候选只要不能证明其 `Severe Error`、`Unsafe Stop`、`Macro-NMAE` 和 `Macro-QWK` 等正式质量指标不劣于对应比较基准，就必须在资源排序前淘汰。更低的成本、更短的延迟、更少的调用或更少的通信不得补偿质量门失败；任何以评分质量下降换取资源下降的结果都不得作为有效研究成果或 Pareto 改进。

基础评分 Agent 的绝对指标可以低于生产部署水平，但其输入语义、Gold、Rubric 和输出必须有效且非退化；在固定 Agent、Prompt、prepared data、预算和评价脚本的公平环境中，A2A-DyGrade-RL 若要声明资源收益，必须先证明评分质量没有下降。
...
**版本**：1.5.0 | **通过日期**：2026-07-04 | **最后修订**：2026-08-14
